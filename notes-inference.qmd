...
$$
Y_i = \beta_0 + \beta_1 X_i + \epsilon_i, \hspace{20pt} \text{where} \hspace{10pt} \epsilon_i \followsp{iid}{N}(0,\sigma^2)
$$
...
Overview
  ...
  - Test on slope $\beta_1$ and the implications
    ...
    + We often want to make inferences about $\beta_1$. A common test on $\beta_1$ has the form:
    ...
    ![](My files/Images/zero-slope.png){width=50%}
...
Sampling distribution of $\hat{\beta}_1$
 ...
 $$
  ...
  - Thus, $S^2_{\hat{\beta}_1}$ is an unbiased estimator for the variance of the sampling distribution of $\hat{\beta}_1$ and
    ...
    + This is also called the **standard error** (another way to think about it is the standard deviation of the sampling distribution) &rarr; $S_{\hat{\beta}_1} = SE(\hat{\beta}_1)$
...
Main result for normal, mean and variance
  ...
  - $\sum \text{Normal} \follow{Normal}$, so we know $\hat{\beta}_1$ must be Normal. Then properties of the coefficients $k_i$ can be used to show the mean and variance.
...
Properties of coefficients $k_i$:
  ...
  ![](My files/Images/ki-properties.png){width=20%}
  
Proof of linear combination:
  
  ![](My files/Images/proof-linear-combination.png){width=50%}
  
Proofs of properties of $k_i$:
  
  ![](My files/Images/proof-ki-properties.png){width=50%}
  
Proof of mean:
  
  ![](My files/Images/proof-mean.png){width=50%}
  
Proof of variance:
  
  ![](My files/Images/proof-variance.png){width=50%}
...
$$
\frac{\hat{\beta}_1 - \beta_1}{S_{\hat{\beta}_1}} = \frac{\hat{\beta}_1 - \beta_1}{\sqrt{MSE / S_{XX}}} \follow{t}_{n - 2}
$$

##### Derivation {-} 

Deriving the distribution of the standardized slope &rarr; $\displaystyle \frac{\hat{\beta}_1 - E(\hat{\beta}_1)}{SE(\hat{\beta}_1)}$

  - Technically when standardizing with an estimated standard deviation it is referred to as *studentized* statistic.

![](My files/Images/placeholder.png){width=50%}
...
Interpretation
  ...
  - With < $1 - \alpha$ >\% confidence, we estimate that the average < $Y$ context > increases by between < lower bound > and < upper bound > for each additional unit increase in < $X$ context >.
  
  - Remember the scope of the regression model is restricted to some range of values of the predictor variable &rarr; May not be reasonable to use these slope estimates outside this range as the regression relation may not be linear then.
...
Overview
  ...
  - If reject &rarr; We conclude that $X$ does contribute information for the prediction of $Y$ when using the straight-line model.
  
  - If fail to reject &rarr; Then we conclude there is no linear relationship between $Y$ and $X$ (horizontal model). But keep in mind:
...
# simulation to determine how the magnitude of beta1 and sigma affect the resulting significance of the X variable in SLR when simulating data
...
Interpretation of confidence coefficient and risks of errors
  ...
  - For example, confidence interval for $\beta_1$ with confidence coefficient 95% &rarr; If many independent samples are taken *where the levels of $X$ are the same as in the dataset*, approximately 95% of the constructed confidence intervals would capture the true value of $\beta_1$.
...
Sampling distribution of $\hat{Y_h}$
  ...
  - Point estimator for $E(Y_h)$ &rarr; $\hat{Y_h} = \hat{\beta}_0 + \hat{\beta}_1 X_h$
...
Notes
  ...
  - Intuitive / visual explanation of this affect
    ...
    ![](My files/Images/variance-y-hat.png){width=40%}
...
$$
\frac{\hat{Y_h} - E(Y_h)}{S_{\hat{Y_h}}} = \frac{\hat{Y_h} - E(Y_h)}{\sqrt{MSE \bigg[\frac{1}{n} + \frac{(X_h - \bar{X})^2}{\sum(X_i - \bar{X})^2}\bigg]}} \follow{t}_{n - 2}
$$

### Confidence interval for $E(Y_h)$ {-} 

  - Goal &rarr; Estimate the mean value of $Y$ for a given value of $X$.

  - Results &rarr; Using the same general format / derivation of a $t$ interval, we have
...
$$
  ...
  - Interpretation &rarr; With < $1 - \alpha$ >\% confidence, we estimate that the true mean value  of < $Y$ context > for all individuals with an < $X$ context > of < $X_h$ > to be between < lower bound > and < upper bound >.

Notes 

  - Interpretations &rarr; Same interpretation rules about repeated sampling for constant $X$ levels (because $X_i$ are known constants in the regression model).
  
  - Confidence interval width is the smallest when $X_h = \bar{X}$ (assuming everything else remains equal).
  
    + Design of experiments &rarr; Thus in an experiment to estimate the mean response at a particular level $X_h$, the precision of the estimate will be best if (everything else remaining equal) the observations on $X$ are spaced so that $\bar{X} = X_h$.
  
  - Tests &rarr; Can use the CI to perform a two-sided test as well.
  
  - Robust &rarr; Confidence limits shown here for $\hat{Y_h}$ are not sensitive to moderate departures from the assumption that the error terms are normally distributed. If there is substantial departures from normality, still not sensitive if large $n$; this robustness comes from robustness of CIs for $\beta_0$ and $\beta_1$.
  ...
  - Multiple intervals &rarr; ** CIs apply when a estimating *a single mean response* from the study. Will show later how to adjust when estimating several mean responses. **
...
Overview
  ...
  - Goal &rarr; Predict a new observation $Y$ for a given $X$ value. This new observation is viewed as the result of a new trial, independent of the trials the model is based on.
...
Demonstration of prediction intervals
  ...
  - If we have a new observation with $X_h = 3.425$ &rarr; $E(Y_h) = 0.10 + 0.95 (3.5) = 3.425$ (so we know the center of the normal distribution of $Y_h$). Thus, using the empirical rule we have the following prediction interval:
  ...
  $$
  ![](My files/Images/simple-prediction-interval.png){width=30%}
...
Overview and demo
  ...
  ![](My files/Images/possible-prediction-intervals.png){width=40%}
...
Prediction interval for $Y_{h(new)}$
  ...
  - Sampling distribution
    ...
    $$
    \frac{Y_{h(new)} - \hat{Y_h}}{S_{pred}} \follow{t}_{n-2}
    $$
  ...
  - Prediction interval
    ...
    + Interpretation &rarr; With < $1-\alpha$ >\% confidence, we predict that the true value of < $Y$ context > for a single (or the next) < $X$ context > of < $X_h$ > to be between < lower bound > and < upper bound >.
...
Standard deviation of prediction $\sigma^2_{pred}$
  ...
  - We can easily find the variance of this difference (because of independence of the new $Y_{h(new)}$ and original $n$ cases on which $\hat{Y_h}$ is based).
	...
	- This has two components:
	  ...
	  1. Variance of the distribution of $Y$ at $X = X_h$ &rarr; $\sigma^2$
	
	  2. Variance of the sampling distribution of $\hat{Y_h}$ &rarr; $\sigma^2_{\hat{Y_h}}$
...
Notes about prediction intervals
  ...
  - Analogous results / interpretations to estimation
    ...
    + Interpretation &rarr; Again, the confidence coefficient refers to taking repeated samples based on the same set of $X$ values, and calculating prediction limits for $Y_{h(new)}$ for each sample.
    
    + Precision &rarr; PI width is the smallest when $X_h = \bar{X}$ (assuming everything else remains equal).
    
    + One interval &rarr; PIs apply for a single prediction based on the sample data.
  
  - Not robust &rarr; PIs (unlike CIs for the mean response) are sensitive to departures from normality of the error terms distribution. Can think of this non-robustness a result of having to take into account the center of the distribution of $Y_h$ (just like with CIs) AND also the tails (spread) of the distribution.
  
  - Precision &rarr; Even if PIs are too wide for useful predictions, they can still be informative for control / modelling purposes, specifically the estimated variance of prediction.
...
Estimation vs Prediction
  ...
  - Location &rarr; For a particular $X_h$, CIs and PIs have the same point estimate $\hat{Y_h}$, which is the estimate of the mean $E(Y_h)$.
  ...
  - Conceptual difference &rarr; CIs resemble PIs, except: A CI represents an inference on a parameter and is an interval that is intended to cover the value of the parameter; and a PI is a statement about the value to be taken by a random variable, the new observation $Y_{h(new)}$.
...
Overview
  ...
  - Goal &rarr; Predict the mean of $m$ new observations on $Y$ for a given level of the predictor variable.

  - Setup &rarr; $\bar{Y}_{h(new)}$ represents the mean of the new $Y$ observations to be predicted.
 ...
 $$
  ...
  - Standard deviation in prediction of a mean $S_{predmean}$
	...
	- This has two components:
	  ...
	  1. Variance of the distribution mean of $m$ observations from the probability distrubtion of $Y$ at $X = X_h$ &rarr; $\sigma^2 / m$
	
	  2. Variance of the sampling distribution of $\hat{Y_h}$ &rarr; $\sigma^2_{\hat{Y_h}}$

Notes
  
  - Interpretation &rarr; With < $1 - \alpha$ >\% confidence, we predict that the true value of < $Y$ context > for < $m$ > < $X$ context > of < $X_h$ > to be between < lower bound > and < upper bound >.
...
Results
  ...
  - The $100(1 - \alpha)\%$ confidence band for the regression line has boundaries at any level $X_h$:
    ...
    + (Initial steps of deriving CI) &rarr; We are finding the two values such that $P(\text{lower} \le \beta_0 + \beta_1 X_h \le \text{upper}) = 1- \alpha$; And $\hat{Y_h}$ is the point estimator for $E(Y_h) = \beta_0 + \beta_1 X_h$.
...
Notes
  ...
  - Confidence band confidence level
    ...
    + Interpretation &rarr; Indicates the proportion of time that the estimating procedure will yield a band that covers the entire line, in a long series of samples in which the $X$ observations are kept at the same level as in the actual study.

    + Technical application &rarr; The confidence band applies to the entire regression line over all real-numbered values of $X$ from $-\infty$ to $\infty$.

    + Practical application &rarr; In practice, the confidence band is ignored for that part of the regression line which is not of interest, so the confidence coefficient for this limited segment is somewhat higher than $1-\alpha$. Thus, $1-\alpha$ serves as a lower bound to the confidence coefficient.
	  
  - Things to look for &rarr; Can see if the slope is clearly positive or negative, can look at the levels of the regression line at different levels of $X$ to gauge relative precision, etc.
  ...
  - Formula &rarr; The lower and upper bounds together actually define a hyperbola.
...
data_conf_band %>% 
  kable(col.names = c("$X_h$", "fit $= \\hat{Y_h}$", "SE(fit) $= S_{\\hat{Y_h}}$", "LB $= \\hat{Y_h} - W \\cdot S_{\\hat{Y_h}}$", "UB = $\\hat{Y_h} + W \\cdot S_{\\hat{Y_h}}$", "Width"),
        digits = 3) %>% 
  kable_styling(full_width = FALSE,
                position = "left")
...
![](My files/Images/ss-breakdown.png){width=100%}

Types of sum of squares

  - Total sum of squares &rarr; Measured in terms of the deviations of the $Y_i$ around their mean $\bar{Y}$.
  ...
  - Sum of squares error &rarr; Measured in terms of the deviations of the $Y_i$ around the fitted regression line $\hat{Y_i}$ (i.e. the residuals $e_i$).
  ...
  - Sum of squares regression &rarr; Measured in terms of the deviations of the fitted $\hat{Y_i}$ around their mean $\bar{Y}$.
...
##### Results {-} 
  ...
  ![](My files/Images/partitioning-deviations.png){width=30%}
...
![](My files/Images/partition-ss-proof.png){width=50%}
...
Degrees of freedom for each sum of square
  ...
  - $SSTO$ &rarr; Has $n-1$ degrees of freedom associated with it.
  ...
  - $SSE$ &rarr; Has $n-2$ degrees of freedom associated with it.
  ...
  - $SSR$ &rarr; Has 1 degrees of freedom associated with it.
...
### Mean squares {-} 
  ...
  - General definition &rarr; A sum of squares divided by its associated degrees of freedom is called a mean square ($MS$).
  ...
  - Note &rarr; Mean squares are not additive $\Longrightarrow$ $MSR + MSE \ne MSTO$
...
Total                $df_{TO} = n -1$ $SSTO = \sum (Y_i - \bar{Y})^2$      
  ...
  - Could convert $SSTO = S_{YY}$ to a mean square as well by dividing by $df_{TO} = n - 1$ &rarr; This gives us $\frac{1}{n - 1}{\sum (Y_i - \bar{Y})^2}$ = Sample variance of $Y_i$ (totally unrelated to the regression line; just the variance of a set of numbers, which is technically a mean square).
...
Goal &rarr; In order to make inferences based on the analysis of variance approach, we need to know the expected value of each of the mean squares.
...
Test on slope
  ...
  - Hypotheses &rarr; For the SLR, ANOVA gives us a test for:
  ...
  - Test statistic &rarr; For ANOVA, the test statistic $F^*$ compares $MSR$ and $MSE$
  
    $$
    TS = F^* = \frac{MSR}{MSE}
    $$
  
      + (see derivation) Under $H_0: \beta_1 = 0$ &rarr; $F^* \follow{F}_{(1, n-2)}$
  ...
  - Rejection region and p-value
    ...
    + Note &rarr; $F_{(1-\alpha;\,1,n-2)}$ is the $100 (1-\alpha)$ percentile of the appropriate $F$ distribution (different notation meaning than $t_{\alpha / 2}$ because not a symmetric distribution now).
    
  - Decision &rarr; Same rules as usual, now just with $F$-distribution
...
Equivalence of $F$ test and $t$ test
  ...
  - The $F$-test is algebraically equivalent to the *two-tailed* $t$ test &rarr; $F^* = (t^*)^2$
  
    + Same relationship for the critical values when defining the rejection region / p-value &rarr; $F_{(1-\alpha; \, 1, n-2)} = t_{\alpha/2, n-2}^2$.
...
Sampling distribution of $F^*$
  ...
  - Goal &rarr; In order to be able to construct a statistical decision rule and examine its properties, we need to know the sampling distribution of $F^*$.
  
  - Derivation &rarr; Start by considering the sampling distribution of $F^*$ under $H_0: \beta_1 = 0$. We will use the following theorem:
  
    + **Cochran's theorem** &rarr; Let $\vecn{Y}{n}$ represent a random sample from the same normal distribution with mean $\mu$ and variance $\sigma^2$. Suppose $SSTO = \sum (Y_i - \bar{Y})^2$ is partitioned into $k$ sums of squares $SS_r$, each with degrees of freedom $df_r$. If $\displaystyle \sum_{r=1}^k df_{r} = n - 1$, then each of the $\frac{SS_r}{\sigma^2}$ terms are independent $\chi^2$ random variables with $df_r$ degrees of freedom.
    
  ![](My files/Images/placeholder.png){width=50%}

  - Under $H_A: \beta_1 \ne 0$
  
    + $F^* \follow{Non-central F}_{(1, n - 2)}$ with non-centrality parameter $\lambda$. 
    + Still $SSR \ind SSE$ and $SSE / \sigma^2 \sim \chi^2_{n-2}$. But the condition that both $SSR / \sigma^2$ and $SSE / \sigma^2$ are $\chi^2$ random variables requires $\beta_1 = 0$.
...
Test statistic and decision
  ...
  - Logic &rarr; Now we compare the two sum of squares from the full and reduced model using the fact that the reduced $SSE$ is always greater than or equal to the full $SSE$.
  
    + $SSE(R) \ge SSE(F)$ &rarr; More parameters in the model ALWAYS leads to a better fit (i.e. *less unexplained* variability =  *more explained* variability) $\Longrightarrow$ Smaller are the deviations around the fitted regression function.
  ...
  - Test statistic is a function of the difference in two $SSE$s, relative to the full $SSE$:
    ...
    + Under $H_0: \beta_1 = 0 \hspace{10pt} \text{Reduced model}$ &rarr; $F^* \follow{F}_{(df_R - df_F, df_F)}$
...
Limitations of $R^2$
  ...
  - Usefulness in prediction &rarr; A high coefficient of determination does not necessarily indicate that useful (precise) predictions can be made.
  ...
  - Quality of fit &rarr; A high coefficient of determination does not necessarily indicate that the estimated regression line is a good fit and similarly an $R^2$ near zero does not necessarily indicate that $Y$ and $X$ are not related.
	  ...
	  ![](My files/Images/r2-limitations.png){width=50%}
	
Inflating $R^2$
  
  - $X$ level spacing &rarr; The value taken by $R^2$ in a given sample tends to be affected by the spacing of the $X$ observations: Wider spacing in $X_i$ $\Longrightarrow$ $R^2$. Here's why:
	...
	- Overfitting &rarr; $R^2$ can be artificially inflated by including additional model terms (adding extra predictors).
...
### Coefficient of correlation {-} 
  ...
  - A measure of linear association between $Y$ and $X$ when both $Y$ and $X$ are *random* is the coefficient of correlation. This is the signed square root of $R^2$:
    ...
    + ONLY is SLR can the coefficient of determination $R^2$ be computed as the square of the correlation coefficient $r^2$.
  ...
  - Note &rarr; Regression models do not contain a parameter to be estimated by $R^2$ or $r$. These are simply *descriptive measures* of the *degree of linear association* between $Y$ and $X$ in the sample observations that *may, or may not, be useful* in any instance.
