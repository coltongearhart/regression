...
Two main ideas
  ...
  - Diagnostics &rarr; Methods to check whether our model is reasonable for our data and representative of the system that we are studying.
  
    + Some diagnostics check the *assumptions* of our model. Other diagnostics check the *influence* of different data points.

  - Remedies &rarr; Analytic strategies used to fix problems identified by the diagnostics.
...
Why do we need to check the model?
  ...
  - All of these are checked via residual analysisl
...
Plots
  ...
  - Dot plot &rarr; Good when the sample size is not large.
  ...
  - Sequence plot &rarr; Should be utilized whenever data are obtained in a sequence, such as over time or for adjacent geographic areas.
  ...
  - Box plots &rarr; Good when have a larger sample size.
	
	  + Can look at min and max, location of IQR, outliers, shape (skewness).
	
  - Histograms &rarr; Good with really big sample sizes.
...
Overview
  ...
  - Goal of residual analysis &rarr; Assess the aptness of a statistical model.
...
### Properites of residuals {-} 
  ...
  - Mean
    ...
    + The mean of the $n$ residuals for the SLR is
  ...
  - Variance
    ...
    + The variance of the $n$ residuals for the SLR is
...
### Semistudentized residuals {-} 
  ...
  - Studentized &rarr; If $\sqrt{MSE}$ were an estimate of the standard deviation of the residual $e_i$, we would call $e_i^*$ a studentized residual. But it is not...
  
  - Semistudentized &rarr; $\sigma_{e_i}$ is complex and varies for the different residuals $e_i$, and $\sqrt{MSE}$ is only an approximation of the standard deviation of $\sigma_{e_i}$. So, instead it is a semistudentized residual.
...
### Departures from model to Be studied by residuals {-} 
  ...
  - We are going to use residuals to study the following departures from the SLR model with normal errors:
    ...
    1. The regression function is not linear &rarr; Is the functional form of the model appropriate?

    2. The error terms do not have constant variance.

    3. The error terms are not independent.

    4. The model fits all but one or a few outlier observations &rarr; Do any of the data points have a disproportionate influence on the parameter estimates?
...
Overview
  ...
  1. *Residual plot against the fitted values* &rarr; This is the preferred plot for this assumption check.
      ...
      ![](My files/Images/diagnostics-linearity.png){width=50%}
  ...
  3. *Scatterplot of $Y$ vs $X$* &rarr; If the actual trend in the data in linear, then it makes sense to fit a straight-line model to the data.
  
      ![](My files/Images/scatterplot-y-vs-x.png){width=50%}
...
Model statement &rarr; $Y_i = \beta_0 + \beta_1 X_i + \text{Normal}(0, \sigma^2)$
...
```
  ...
  - Residuals vs $X$ plot
    ...
    + For SLR &rarr; Shows same pattern as above, different scale on $X$ axis.
...
```
  ...
  - Demo for difference when in MLR 
    ...
    + Results &rarr; Can see different pattern (even if only slightly based on values chosen) pattern in each.
...
Model statement &rarr; $Y_i = \beta_0 + \beta_1 X_i + \beta_2 X_i^2 + \text{Normal}(0, \sigma^2)$
...
```
  ...
  - Demo to show how steep slope can hide lack of fit in scatterplot, but residual plot still picks it up. 
    ...
    + Results &rarr; For a true quadratic model, the scatterplot of $Y$ vs $X$ looks just linear (not noticeable curvature) when there is a large $\beta_1$ term (coefficient of linear term). But this is misleading, especially after seeing the residual plot which shows the missing evidence of curvature.
...
Model statement &rarr; $Y_i = \beta_0 \, \e^{\beta_1 \, X_i} + \text{Normal}(0, \sigma^2)$
...
Overview
  ...
  1. *Residual plot against the fitted values*
      ...
      ![](My files/Images/diagnostics-nonconstant-variance.png){width=50%}
  
  2. *Scale-location plot* &rarr; Refined version of above, preferred plot for this assumption.
     ...
     ![](My files/Images/diagnostics-nonconstant-variance-scale-location.png){width=50%}
     ...
     + By doing this, it places all of the information on changing magnitudes of the residuals above the horizontal zero line, which makes it easier to see whether the magnitude of the residuals is changing with the level $\hat{Y}$ ($\pm$ is not important for this condition).
      ...
      ![](My files/Images/diagnostics-scale-location-standardized-residuals.png){width=50%}
...
Model statement &rarr; $Y_i = \beta_0 + \beta_1 X_i + \text{Normal}(0, \sigma^2_i = 5 X_i)$
...
Model statement &rarr; $Y_i = \beta_0 + \beta_1 X_i + \text{Normal}(0,  \text{ decreasing then increasing } \sigma^2_i)$
...
Overview
  ...
  - Unusual observations can create much difficulty when fitting models. When present, they can lead to a misleading fit because the line was estimated by minimizing the squared deviations.
    ...
    + Thus, a fitted line may be pulled disproportionately toward an unusual observation. Unusual observations near far from $\bar{X}$ have a larger impact on the model and estimates than those near $\bar{X}$ (this is the idea of leverage / influence).
    
    ![](My files/Images/outliers-relative-xbar.png){width=30%}

  - There are two types unusual observations and how we define them, check for them, and their impact is different.
  
    1. **Outliers** &rarr; Extreme values of the response (outliers in $Y$).
   
      + A rough rule of thumb &rarr; When the sample size is large, semistudentized (or standardized) residuals with absolute value more than 3 or 4 can be considered outliers (i.e. outliers if $\lvert \frac{e_i}{\sqrt{MSE}} \text{ or } e_i^* \rvert \ge 3 \text{ or } 4$).
      ...
      ![](My files/Images/diagnostics-outlier-y.png){width=30%}
      ...
      ![](My files/Images/diagnostics-outlier-semistudentized.png){width=30%}
      ...
      ![](My files/Images/diagnostics-outliers-other-plots.png){width=50%}
  
    2. **High leverage points** &rarr; Observations whose predictor values are far from the center of the predictor space (extreme values (outliers) of $X$ or unusual combination of $X$s).
    
      c. Plots &rarr; *Residuals vs Leverage plot* (or Cook's distance plot, Cook's dist vs Leverage (1 - Leverage) plot) &rarr; These use measures not covered yet.
      
    (3.) **Influential points** &rarr; High leverage points that actually *influence* the slope of the regression line (outlier in $X$ AND $Y$).
      
      d. Plots &rarr; In order to determine if a point is influential, visualize the regression line with and without the point. Does the slope of the line change considerably? If so, then the point is influential. If not, then it’s not an influential point.
      
      ![](My files/Images/diagnostics-high-leverage-influential.png){width=50%}
...
Removing unusual observations
  ...
  - A safe rule &rarr; Discard an unusual observation only if there is direct evidence that it represents an error in recording, a miscalculation, a malfunctioning of equipment, or a similar type of circumstance.
...
data_plot %>% 
  ...
  scale_color_manual(name = "Type of residaul",
...
Model statement &rarr; $Y_i = \beta_0 + \beta_1 X_i + \text{Normal}(0, \sigma^2), \hspace{20pt} \text{except} \hspace{10pt} Y_{n+1} = \beta_0^* + \beta_1^* X_i + \text{Normal}(0, \sigma^2)$
...
Overview
  ...
  - *Sequence plot of residuals*
    ...
    ![](My files/Images/diagnostics-sequence-plot-time.png){width=70%}
...
Modelling with nonindependence or apparent nonindependence
  ...
  ![](My files/Images/diagnostics-residual-plot-squared-missing.png){width=30%}
...
Model statement &rarr; $Y_i = \beta_0 + \beta_1 X_i + \epsilon_i, \hspace{20pt} \text{where} \hspace{10pt} \epsilon_i \follow{Normal}(\mu = \epsilon_{i - 1}, \sigma^2)$
...
Model statement &rarr; $Y_i = \beta_0 + \beta_1 X_i + \epsilon_i, \hspace{20pt} \text{where} \hspace{10pt} \epsilon_i \follow{Normal}(\text{increasing } \mu, \sigma^2)$
...
```
  ...
  - Sequence plot with linear trend effect (still dependent).
    ...
    + Clear increase from in residuals with collection order &rarr; Early points more have negative residuals and later ones are positive.
...
Model statement &rarr; $Y_i = \beta_0 + \beta_1 X_i + \epsilon_i, \hspace{20pt} \text{where} \hspace{10pt} \epsilon_i \follow{Normal}(\text{cyclical } \mu, \sigma^2)$
...
Overview
  ...
  1. *Distribution plots of residuals* &rarr; This is one of the two preferred plots.
  ...
  2. *Comparison of relative frequencies* &rarr; Use the empirical rule to compare observed relative frequencies to expected under normality.
     ...
     ![](My files/Images/diagnostics-normality-empirical-rule.png){width=50%}
    
  3. *Normal probability (QQ) plot of residuals* &rarr; This is the other preffered plot.
      ...
      ![](My files/Images/diagnostics-qq-plots-histograms.png){width=80%}
...
Model statement &rarr; $Y_i = \beta_0 + \beta_1 X_i + \text{Exponential}(\lambda = 1/5)$
...
Model statement &rarr; $Y_i = \beta_0 + \beta_1 X_i + \text{Uniform}(a = -2, b = 2)$
...
# generate heavy-tailed error terms from uniform distribution
...
```
  ...
  - Same methods &rarr; Relative frequencies don't line up; likely no outliers on boxplot; the histogram may show more data closer to the center; S-shape in QQ plot.
...
Model statement &rarr; $Y_i = \beta_0 + \beta_1 X_i + \text{t}_4$
...
```
  ...
  - Same methods &rarr; Again, relative frequencies don't line up; likely outliers on both sides of boxplot; the histogram will may show more data away from the center; flipped S-shape in QQ plot.

```{r}

# comparison of frequencies
mse <- summary(mod_heavy_tailed)$sigma^2
e <- resid(mod_heavy_tailed)
c(1:3) %>% 
  set_names(c("± sqrt(MSE)", "± 2 sqrt(MSE)","± 3 sqrt(MSE)")) %>% 
  map_dbl(\(z) round(mean(abs(e) <= z * sqrt(mse)), 3))

# boxplot of residuals
boxplot(resid(mod_heavy_tailed), horizontal = TRUE,  main = "Heavy-tailed error")

# histogram of standarized residuals (so can overlay Z density curve)
hist(rstandard(mod_heavy_tailed), freq = FALSE, breaks = 20, main = "Heavy-tailed error")
x_plot <- seq(from = -3, to = 3, by = 0.01)
lines(x = x_plot, dnorm(x = x_plot))

# normal qq plot
plot(mod_heavy_tailed, which = 2)
...
Overview
  ...
  - Residuals should also be plotted against variables omitted from the model that might have important effects on the response.
    ...
    ![](My files/Images/diagnostics-residual-vs-omittted-x.png){width=50%}
...
Model statement &rarr; $Y_i = \beta_0 + \beta_1 X_{1,i} + 0 \, X_{2,i} + \text{Normal}(0, \sigma^2)$
...
Model statement &rarr; $Y_i = \beta_0 + \beta_1 X_{1,i} + \beta_2 X_{2,i} + \text{Normal}(0, \sigma^2)$
...
##### Overview {-} 
  ...
  - If the normal error SLR model is not appropriate for a dataset, there are two basic choices, both of which have their advantages and disadvantages:
    ...
    1. Abandon this model and develop and use a more appropriate model.
      ...
      + Pro &rarr; May lead to a more complex model that could yield better insights.
      
      + Con &rarr; May also lead to more complex procedures for estimating the parameters.
    
    2. Use some transformation on the data so that the normal error SLR model is appropriate for the transformed data.
        
      + Pro &rarr; Leads to relatively simple methods of estimation with smaller models (less parameters), which is desirable when the sample size is small.
      
      + Con &rarr; Transformations may obscure the fundamental interconnections between the variables, though at other times they may illuminate them.
      
    - Now we consider only transformations (will cover more complex models in later chapters).
      
      ![](My files/Images/remedial-measures-summary.png){width=50%}
...
Nonlinearity of the regression function
  ...
  - When the regression function is not linear, a direct approach is to modify the SLR regression model by altering the nature of the regression function (these methods will be discussed much later). For example,
    ...
    + Quadratic regression function (this is an example of a polynomial regression function) &rarr; $E(Y) = \beta_0 + \beta_1 X + \beta_2 X^2$
    
    + Exponential regression function (another example of a nonlinear regression function) &rarr; $E(Y) = \beta_0 \cdot \beta_1^X$
...
Overview
  ...
  - Goal &rarr; Linearize a nonlinear regression relation.

  - Use when &rarr; Distribution of error terms is reasonably close to a normal distribution AND have approximately constant variance.

  - Strategy &rarr; Transformations on $X$ should be attempted.
	  ...
	  + e.g.) $Y = \beta_0 + \beta_1 X + \epsilon$, where $\epsilon \follow{Normal}$ $\rightarrow$ $\sqrt{Y} = \beta_0 + \beta_1 X + \text{new }\epsilon$
...
Specific transformations
  ...
  - Below are some prototype nonlinear regression relations with some simple transformations on $X$ that may be helpful for linearizing without affecting the distribution of $Y$.
    ...
    + Several transformations can be tried.
	  ...
	  ![](My files/Images/transformations-nonlinear.png){width=50%}
...
```
  ...
  - Scatterplot of relationship between $Y$ and $X$
    ...
    + Main issue &rarr; See curvilinear pattern (in this case $Y$ grows slower as $X$ increases). So standard SLR model not appropriate (note that in reality, we have no idea what type model this came from).
    
    + Roughly same variance in $Y$ as $X$ increases (imagining spread of observations relative to the smooth curve; the residual plot looks bad because of the lack of fit) and QQ plot looks decent enough &rarr; So only issue is the linearity, which means we want to transform $X$ based on the specific pattern.
...
```
  ...
  - Attempt two transformations chosen based on the observed pattern: 1) $X' = X^2$ and 2) $X' = \e^X$.
...
```
  ...
  - Attempt two transformations chosen based on the observed pattern: 1a) $X' = 1 / X$ 1b) $X' = 1 / (X + k)$ and 2) $X' = \e^{-X}$.
  
    + $1 / X$ does not result in a linear relationship, so no need to continue with that. However all $X$ values are in between 0 and 1, so we can try to introduce a constant to move data away from origin, say $X' = 1 / (1 + X)$. This works better. $\e^{-X}$ seems to helps as well.
...
Results
  ...
  - Coefficients
    ...
    + Adding constants (all combos $X + k$, $Y + k$ and both) &rarr; Changes intercept, doesn't affect slope.
    
    + Multiplying by scalar &rarr; $k \cdot X$ doesn't affect intercept, but does affect slope; $k \cdot Y$ does affect intercept and slope; Both $k \cdot X$ and $k \cdot Y$ has same intercept as only $k \cdot Y$ (because $k \cdot X$ doesn't affect it), but the same slope as the original $X$ and $Y$ (in essence cancelling each others' effect).
...
$log_{10}(X)$ and $\ln(X)$
  ...
  ![](My files/Images/transformations-log10-vs-ln.png){width=50%}
...
Overview
  ...
  - Goal &rarr; Fix nonnormality and nonconstant variance of regression relation.

  - Strategy &rarr; We need a transformation on $Y$ because the shapes and spreads of the distributions of $Y$ need to be changed. 
...
Specific transformations
  ...
  - Below are some prototype regression relations with some simple transformations on $Y$ that may be helpful for fixing non normality and unequal variances of the error distributions.
    ...
    ![](My files/Images/transformations-nonnormal-and-nonconstant-var.png){width=50%}
...
epsilon <- runif(n = n, min = 0, max = x/4)

# calculate observations Y
# -> structurally should have linear relationship in population and the other violations are what make appear nonlinear
# -> but for simulating, just using same form as nonlinear departure
y <- (beta_0 + beta_1 * x)^0.3 + epsilon

# scatter plot with poorly fit regression line and smoothed curve added
plot(x = x, y = y, main = "Nonnormal errors with increasing variance \n ~ Increasing to an asymptote pattern")
abline(lm(y ~ x), col = "red")
lines(lowess(x, y), col = "blue")
...
```
  ...
  - Attempt transformations chosen based on the observed pattern: $Y' = \sqrt{Y}$. Then view transformed scatterplot.
    ...
    <!-- NOTE: detaching -> skipping simulations for the rest of the prototypes so can move on to new stuff -->
    
```{r}
#| echo = FALSE

# view scatterplot and regression lines for potential transformations
plot(x = x, y = sqrt(y), main = "Transformation = sqrt(Y)")
abline(lm(sqrt(y) ~x), col = "red")
plot(x = sqrt(x), y = sqrt(y), main = "Transformation = sqrt(Y)")
abline(lm(sqrt(y) ~ sqrt(x)), col = "red")
...
## (come up with new title) Using the transformed model 
